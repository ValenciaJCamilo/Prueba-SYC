...
if (empty($documento_cliente) || empty($_POST['valor'])) {
  echo "<script>window.alert('Por favor complete los campos que son obligatorios.'); window.history.go(-1);</script>";
  exit();
}
...
if(mysqli_num_rows($result) > 0) {
    ...
    if($query) {
      echo "<script>window.alert('La factura ha sido creada exitosamente!'); window.location.href='../index.php';</script>";
    } else {
